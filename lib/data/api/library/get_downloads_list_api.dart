...
getDownloadBookApi({pagination}) async {
  DownloadController con = Get.put(DownloadController());
  try {
    bool connection =
    ...
    if (connection) {
      ...
      if (LocalStorage.token.toString() != "null" &&
        ...
        if (response.statusCode == 200 ||
          ...
          var decoded = getDownloadBookModelFromJson(response.body);
          if (decoded.data!.isNotEmpty) {
            for (var element in decoded.data!) {
              con.downloadList.add(element);
            }

            con.page.value++;
          }

          if (con.downloadList.length.toString() ==
